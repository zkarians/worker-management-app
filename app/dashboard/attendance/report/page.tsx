                    ...
                    rosterData.roster.assignments.forEach((assignment: any) => {
                        ...
                        if (stats) {
                            ...
                            if (!hasRecord) {
                                ...
                                if (targetDate < today) {
                                ...
                                } else if (targetDate.getTime() === today.getTime() && now.getHours() >= 19) {
                                    // Today after 19:00: Mark as PRESENT with 8 work hours
                                    stats.presentDays++;
                                    stats.totalDays++;
                                    stats.totalWorkHours += 8;
                                    ...
                                    // Today before 19:00: Mark as SCHEDULED
                                    stats.scheduledDays++;
                                    stats.totalDays++;
            ...
            if (rosterData.roster?.assignments) {
                ...
                rosterData.roster.assignments.forEach((assignment: any) => {
                    ...
                    if (!hasRecord) {
                        ...
                        if (targetDate < today || (targetDate.getTime() === today.getTime() && now.getHours() >= 19)) {
                            processedData.push({
                                userId: assignment.userId,
                                date: detailDate,
                                status: 'PRESENT',
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-gradient-to-r from-slate-100 to-slate-50 border-b-2 border-slate-300">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-center text-sm font-bold text-slate-700">휴가</th>
                            ...
                            {loading ? (
                                ...
                                userStats.map((stat) => (
                                    <tr key={stat.userId} className="hover:bg-slate-50/80 transition-all duration-200">
                                        ...
                                        <td className="px-4 py-3 text-center">
                                            <span className="px-3 py-1.5 rounded-lg text-sm font-bold bg-teal-100 border border-teal-300 text-teal-700">
                                                {stat.vacationDays}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3 text-center">
