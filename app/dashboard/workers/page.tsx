...
import { Users, Search, Filter, Check, X, Edit2, Trash2, History, Printer } from 'lucide-react';
...
export default function WorkersPage() {
    ...
    const fetchWorkers = async () => {
        try {
            const res = await fetch('/api/users?includeResigned=true');
            const data = await res.json();
            if (data.users) {
    ...
    const handleEdit = (user: User) => {
        ...
        setEditForm({
            name: user.name,
            companyId: user.company ? (companies.find((c: { id: string; name: string }) => c.name === user.company?.name)?.id || '') : '',
    ...
    const handleResign = async (user: User) => {
        const dateStr = prompt(`${user.name}님의 퇴사일을 입력해주세요 (YYYY-MM-DD):`, new Date().toISOString().split('T')[0]);
        if (!dateStr) return;

        if (!confirm(`${user.name}님을 ${dateStr}부로 퇴사 처리하시겠습니까?\n퇴사 처리 시 더 이상 접속할 수 없으며 근무표에서 제외됩니다.`)) return;
    ...
    const handleCancelResign = async (user: User) => {
        if (!confirm(`${user.name}님의 퇴사 처리를 취소하시겠습니까?`)) return;
        ...
        try {
            ...
            console.error('Cancel resign failed', error);
            alert('처리 실패');
        }
    };
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2 print:hidden">
                    <button
                        onClick={handlePrint}
                        className="btn-secondary glass-button bg-slate-800 hover:bg-slate-700 text-white border-none flex items-center gap-2"
                    >
                        <Printer size={18} /> 인쇄
                    </button>
                    <button
                        onClick={() => setIsAddingUser(true)}
                        className="btn-primary glass-button bg-indigo-600 hover:bg-indigo-500 flex items-center gap-2"
                    >
                        <Users size={18} /> + 사용자 추가
                    </button>
                </div>
            </div>
            ...
            <div className="hidden print:block mb-8 text-center">
                <h1 className="text-3xl font-bold text-slate-900 mb-2">근무자 명단 관리 현황</h1>
                <p className="text-slate-500">출력일시: {new Date().toLocaleString('ko-KR')}</p>
            </div>

            {/* Search and Filter */}
            <GlassCard className="flex gap-4 bg-white border-slate-200 print:hidden">
                ...
                <table className="w-full text-left text-sm text-slate-500">
                    <thead className="bg-slate-50 text-slate-700 uppercase font-medium border-b border-slate-200">
                        <tr>
                            ...
                            <th className="px-6 py-4 text-right print:hidden">작업</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200">
                        {filteredWorkers.map((worker: User) => (
                            <tr key={worker.id} className={`hover:bg-slate-50 transition-colors ${worker.resignationDate ? 'bg-gray-50 opacity-75' : ''}`}>
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold print:hidden ${worker.role === 'MANAGER'
                                        ...
                                        <div>
                                            <p className="font-medium text-slate-900 flex items-center gap-2">
                                                {worker.name}
                                                {worker.resignationDate && <span className="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full">퇴사</span>}
                                            </p>
                                ...
                                <td className="px-6 py-4 text-slate-700">{worker.email || '-'}</td>
                                <td className="px-6 py-4 text-slate-700">
                                    {worker.hireDate ? new Date(worker.hireDate).toLocaleDateString() : '-'}
                                    {worker.resignationDate && (
                                        <div className="text-xs text-red-500 mt-1">
                                            퇴사: {new Date(worker.resignationDate).toLocaleDateString()}
                                        </div>
                                    )}
                                </td>
                                ...
                                <td className="px-6 py-4 text-right print:hidden">
                                    <div className="flex items-center justify-end gap-2">
                                        ...
                                        {worker.resignationDate ? (
                                            <button
                                                onClick={() => handleCancelResign(worker)}
                                            ...
                                            <button
                                                onClick={() => handleResign(worker)}
                                                className="p-2 rounded-lg hover:bg-orange-100 text-slate-400 hover:text-orange-600 transition-colors"
                                                title="퇴사 처리"
                                            >
            ...
            < div className="md:hidden space-y-3" >
                {
                    filteredWorkers.length === 0 ? (
                        ...
                        filteredWorkers.map((worker: User) => (
                            <GlassCard key={worker.id} className={`bg-white border-slate-200 p-4 ${worker.resignationDate ? 'opacity-75 bg-gray-50' : ''}`}>
                                <div className="space-y-3">
                                    ...
                                    <div className="grid grid-cols-2 gap-3 pt-2 border-t border-slate-200">
                                        ...
                                        <div className="space-y-1 col-span-2">
                                            ...
                                            <p className="text-sm text-slate-700 font-medium">{worker.email || '-'}</p>
                                        </div>
                                        <div className="space-y-1 col-span-2">
                                        ...
                                        {worker.resignationDate && (
                                            <div className="space-y-1 col-span-2">
                                                <p className="text-xs text-red-500 font-medium">퇴사일</p>
                                                <p className="text-sm text-red-700 font-medium">
                                                    {new Date(worker.resignationDate).toLocaleDateString('ko-KR', {
                                                        year: 'numeric',
                                                        month: 'long',
                                                        day: 'numeric'
                                                    })}
                                    ...
                                    <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-200">
                                        {!worker.isApproved && (
                                            <button
                                                onClick={() => handleApprove(worker.id, true)}
                                                className="w-full px-3 py-2 rounded-lg bg-green-100 text-green-700 hover:bg-green-200 transition-colors text-sm font-medium flex items-center justify-center gap-2"
                                            >
                                                <Check size={16} />
                                                승인
                                            </button>
                                        ...
                                        {worker.isApproved && (
                                            <button
                                                onClick={() => handleApprove(worker.id, false)}
                                                className="w-full px-3 py-2 rounded-lg bg-yellow-100 text-yellow-700 hover:bg-yellow-200 transition-colors text-sm font-medium flex items-center justify-center gap-2"
                                            >
                                                <X size={16} />
                                                승인취소
                                            </button>
                                        ...
                                        <button
                                            onClick={() => handleEdit(worker)}
                                            className="w-full px-3 py-2 rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200 transition-colors text-sm font-medium flex items-center justify-center gap-2"
                                        >
                                            <Edit2 size={16} />
                                            수정
                                        </button>
                                        {worker.resignationDate ? (
                                            <button
                                                onClick={() => handleCancelResign(worker)}
                                                className="w-full px-3 py-2 rounded-lg bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors text-sm font-medium flex items-center justify-center gap-2"
                                            >
                                                <History size={16} />
                                                복직
                                            </button>
                                            ...
                                            <button
                                                onClick={() => handleResign(worker)}
                                                className="w-full px-3 py-2 rounded-lg bg-orange-100 text-orange-700 hover:bg-orange-200 transition-colors text-sm font-medium flex items-center justify-center gap-2"
                                            >
                                                <X size={16} className="rotate-45" />
                                                퇴사
                                            </button>
                                        ...
                                        <button
                                            onClick={() => handleDeleteUser(worker)}
                                            className="w-full px-3 py-2 rounded-lg bg-red-100 text-red-700 hover:bg-red-200 transition-colors text-sm font-medium flex items-center justify-center gap-2"
                                        >
                                            <Trash2 size={16} />
                                            삭제
                                        </button>
