...
export default function ProductsPage() {
    ...
    const [loadMode, setLoadMode] = useState<'search' | 'all' | null>(null);
    ...
    }, [searchTerm, loadMode, currentPage]);
    ...
    const handleDownloadAll = async () => {
        try {
            ...
            const exportData = data.products.map((item: any) => ({
                '제품명': item.name,
                '카테고리': item.category?.name || '미분류',
                '가로': item.width,
                '세로': item.depth,
                '높이': item.height,
                '무게': item.weight || '',
            ...
            XLSX.writeFile(wb, '전체_제품정보_다운로드.xlsx');
        } catch (error) {
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2 flex-wrap">
                    <Package /> 제품 정보
                    <span className="ml-2 text-sm font-medium text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full whitespace-nowrap">
                        전체 {totalCount}건
                        {searchTerm && <span className="text-indigo-600 ml-1">(검색: {products.length}건)</span>}
                    </span>
                </h1>
                    ...
                    {isManager && (
                        <>
                            ...
                            <button
                                ...
                                className="glass-button bg-white text-indigo-600 border-indigo-200 hover:bg-indigo-50 flex items-center gap-2"
                                title="전체 다운로드"
                            >
                                <Download size={18} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                ...
                {!loading && products.length === 0 && (
                    <div className="col-span-full py-12 text-center text-slate-400 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
                        ...
                        <p>{loadMode === 'search' && !searchTerm ? '검색어를 입력해 제품을 찾아주세요.' : '등록/검색된 제품이 없습니다.'}</p>
                    </div>
                )}
            </div>
            ...
            {totalCount > itemsPerPage && (
                <div className="flex justify-center items-center gap-2 mt-8 pb-8">
                    <button
                        ...
                        className="glass-button bg-white text-slate-600 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        이전
                    </button>
                    <div className="flex gap-1">
