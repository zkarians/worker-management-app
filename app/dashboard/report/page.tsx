...
import { Printer, FileText, Users, CheckCircle, XCircle, TrendingUp, Building2, ChevronLeft, ChevronRight, Camera, Calendar } from 'lucide-react';
...
export default function ReportPage() {
    ...
    const [companyStats, setCompanyStats] = useState<{ [key: string]: { total: number; present: number; absent: number; leave: number } }>({});
        ...
        try {
            ...
            if (usersData.users) {
                ...
                const vacationOrLeaveUserIds = new Set(
                    (attData.attendance || [])
                        .filter((a: any) => a.status === 'VACATION' || a.status === 'LEAVE_OF_ABSENCE')
                        .map((a: any) => a.userId)
                );

                const vacationOrLeaveCount = vacationOrLeaveUserIds.size;
                ...
                const companyStatsMap: { [key: string]: { total: number; present: number; absent: number; leave: number } } = {};
                ...
                allUsers.forEach((u: any) => {
                    ...
                    if (!companyStatsMap[companyName]) {
                        companyStatsMap[companyName] = { total: 0, present: 0, absent: 0, leave: 0 };
                    }
                    companyStatsMap[companyName].total++;

                    if (assignedWorkerIds.has(u.id)) {
                        companyStatsMap[companyName].present++;
                    } else if (vacationOrLeaveUserIds.has(u.id)) {
                        companyStatsMap[companyName].leave++;
                    } else {
    ...
    const handleScreenshot = async () => {
        ...
        try {
            ...
            // 현재 적용된 zoom 값을 계산하기 위해 computed style 확인
            const computedStyle = window.getComputedStyle(node);
            let zoom = 1;

            // zoom 값이 '1.5' 또는 '150%' 형태일 수 있으므로 파싱 처리
            if (computedStyle.zoom && computedStyle.zoom !== 'normal') {
                const zoomValue = parseFloat(computedStyle.zoom);
                if (computedStyle.zoom.includes('%')) {
                    zoom = zoomValue / 100;
                } else {
                    zoom = zoomValue;
                }
            ...
            console.log('Detected Zoom:', zoom, 'Computed:', computedStyle.zoom);

            // zoom이 적용된 실제 크기 계산
            // scrollWidth/Height는 zoom이 적용되지 않은 원래 크기를 반환할 수 있으므로 zoom을 곱해줍니다.
            const width = node.scrollWidth * zoom;
            const height = node.scrollHeight * zoom;
            ...
            const blob = await toBlob(node, {
                ...
                // 캔버스 크기를 zoom이 적용된 크기로 설정
                width: width,
                height: height,
                style: {
                    // html-to-image는 비표준 css zoom 속성을 완벽하게 지원하지 않을 수 있습니다.
                    // 따라서 캡처 시에는 zoom을 1로 초기화하고, 표준 속성인 transform: scale()로 대체하여
                    // 동일한 확대 효과를 내면서도 짤림 없이 캡처되도록 합니다.
                    zoom: '1',
    ...
    return (
        <div className="pb-20 print:pb-0 print:bg-white print:text-black min-h-screen bg-slate-50 relative z-0">
            ...
            <div className="w-full overflow-x-auto pb-10 px-4 md:px-0">
                <div ref={reportRef} className="pc-zoom-150 min-w-[210mm] mx-auto bg-white text-black p-[15mm] shadow-2xl min-h-[297mm] print:shadow-none print:w-full print:max-w-none print:min-h-0 print:p-[5mm] print:m-0 print:max-h-[285mm] print:overflow-hidden flex flex-col box-border">
                    ...
                    <div className="flex-1 flex flex-col gap-2 print:gap-1 min-h-0">
                        ...
                        <div className="flex-shrink-0">
                            ...
                            <div className="grid grid-cols-5 gap-2 print:gap-1">
                                ...
                                <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg border border-purple-200 p-2 shadow-sm print:p-1 print:rounded">
                                    <div className="flex items-center justify-between mb-1 print:mb-0">
                                        <span className="text-[10px] font-medium text-purple-700 print:text-[8px]">휴가/휴직</span>
                                        <Calendar size={12} className="text-purple-500 print:hidden" />
                                    </div>
                                    <div className="text-lg font-bold text-purple-700 print:text-sm">{stats.leave}명</div>
                                </div>
                                    ...
                                    <table className="w-full border-collapse text-center text-xs print:text-[8px]">
                                        <thead>
                                            <tr className="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200">
                                                ...
                                                <th className="p-1.5 font-semibold text-slate-700 print:p-0.5">휴가/휴직</th>
                                        ...
                                        <tbody>
                                            {Object.entries(companyStats).map(([companyName, stats], index) => (
                                                <tr key={companyName} className={`border-b border-slate-100 ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}>
                                                    ...
                                                    <td className="p-1.5 font-semibold text-purple-600 print:p-0.5">{stats.leave}명</td>
                                                ...
                                                {leaveOfAbsenceWorkers.length > 0 && (
                                                    <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden print:rounded print:border-slate-300">
                                                        <div className="bg-gradient-to-r from-purple-50 to-purple-100 p-1.5 border-b border-purple-200 print:p-1">
                                                            <h3 className="text-[10px] font-bold text-slate-800 flex items-center gap-1 print:text-[8px]">
                                                                 <div className="w-0.5 h-3 bg-purple-500 rounded-full print:h-2"></div>
                                                                 휴직 ({leaveOfAbsenceWorkers.length}명)
                                                            </h3>
                                                        </div>
                                                        <div className="p-1.5 print:p-1">
                                                            <div className="flex flex-wrap gap-1 print:gap-0.5">
                                                                 {leaveOfAbsenceWorkers.map((worker: any, idx: number) => {
                                                                     const style = getCompanyStyle(worker.user?.company?.name);
                                                                     return (
                                                                         <div
                                                                             key={idx}
