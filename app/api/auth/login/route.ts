    ...
    try {
        ...
        try {
            ...
            if (dbError.code === 'P1001') {
                return jsonResponse({
                    error: `데이터베이스 서버 연결 불가 (P1001): ${dbError.message}`
                }, 503);
            }

            if (dbError.message?.includes('timeout')) {
                return jsonResponse({
                    error: `데이터베이스 연결 시간 초과: ${dbError.message}`
                }, 503);
            }
        ...
        try {
            await login({ userId: user.id, role: user.role, name: user.name, username: user.username }, body.rememberMe);
        } catch (loginError) {
        ...
        return jsonResponse({
            ...
            user: { id: user.id, role: user.role, name: user.name, username: user.username }
        });
    } catch (error: any) {
        ...
        if (error.code === 'P1001' || error.message?.includes('Can\'t reach database server')) {
            return jsonResponse({
                error: `데이터베이스 연결 오류 (${error.code || 'Unknown'}): ${error.message}`
            }, 503);
        }
        ...
        if (error.code && error.code.startsWith('P')) {
            return jsonResponse({
                error: `데이터베이스 오류 (${error.code}): ${error.message}`
            }, 500);
        }
